...
import { FiPlus, FiEdit2, FiTrash2, FiLayers, FiArrowLeft } from 'react-icons/fi';
import type {
  PecaGrupoCatalogo,
  ...
  const saveCat = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        imagem_url: catForm.imagem_url.trim() || null,
      ...
      const data = await res.json();
      if (!res.ok || !data.ok) {
        setMessage({ type: 'error', text: data.error || 'Erro ao salvar categoria' });
        return;
      }
      setMessage({
        type: 'success',
        text: editingCat ? 'Categoria atualizada!' : 'Categoria criada!',
      });
      ...
      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Hierarquia: Grupo → Categoria → Subcategoria (ex: Telas → iPhone → OLED).
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {grupos.map((grupo) => {
            ...
            return (
              <div key={grupo.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      ...
      {showCatModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-4">
          ...
          >
            ...
            <label className="block">
              <span className="text-xs font-medium text-gray-600">URL do logo</span>
              <input
                value={catForm.imagem_url}
                onChange={(e) => setCatForm((f) => ({ ...f, imagem_url: e.target.value }))}
                placeholder="https://... (opcional — iPhone/Samsung já têm logo padrão)"
                className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
              />
            </label>
            <label className="flex items-center gap-2 text-sm">
            ...
            <div className="flex gap-2 pt-2">
              <button type="button" onClick={() => setShowCatModal(false)} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm">
                Cancelar
              </button>
              <button type="submit" disabled={savingCat} className="flex-1 px-4 py-2.5 bg-black text-white rounded-lg text-sm disabled:opacity-50">
