...
export default function ContasAPagarPage() {
  ...
  const [filtroMes, setFiltroMes] = useState(() => {
    ...
    console.log('🗓️ Inicializando filtroMes com:', mesAtual);
    return mesAtual;
  });
  ...
  const filteredContas = contasComFixasVirtuais.filter(conta => {
    ...
    if (conta.conta_fixa) {
      console.log(`🔍 Filtro conta fixa: ${conta.descricao}`, {
        ...
        matchesStatus,
        matchesMes,
        contaMes: filtroMes ? new Date(conta.data_vencimento).toISOString().slice(0, 7) : 'N/A',
        filtroMes,
        matches,
        activeTab,
        tipo: conta.tipo
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{contas.length}</p>
              </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 mb-6">
          <div className="flex border-b border-gray-200">
            {[
              ...
              { key: 'fixas', label: 'Fixas', count: contas.filter(c => c.tipo === 'fixa').length },
              { key: 'variaveis', label: 'Variáveis', count: contas.filter(c => c.tipo === 'variavel').length },
              { key: 'pecas', label: 'Peças', count: contas.filter(c => c.tipo === 'pecas').length }
            ].map(tab => (
              <button
                key={tab.key}
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {contasDoMes.length} conta{contasDoMes.length !== 1 ? 's' : ''} • 
