...
export default function DashboardPage() {
  ...
  // Estado para controlar edição de título de coluna
  const [colunaEditandoIndex, setColunaEditandoIndex] = useState<number | null>(null);
...
// Função antiga para compatibilidade e uso no EditableColunaNome
  ...
  // Cálculo do progresso das notas concluídas
  const totalNotas = notes.length;
  const notasConcluidas = notes.filter((nota) => nota.concluida).length;
  const percentualConcluido = totalNotas > 0 ? (notasConcluidas / totalNotas) * 100 : 0;
                          ...
                          <>
                            ...
                            >
                              <div className="flex items-center gap-2">
                                {colunaEditandoIndex === index ? (
                                  <input
                                    type="text"
                                    value={colunas[index]}
                                    onChange={(e) => handleColunaTituloChange(index, e.target.value)}
                                    onBlur={() => { salvarTituloColuna(index); setColunaEditandoIndex(null); }}
                                ...
                                <button
                                  type="button"
                                  onClick={(e) => { e.stopPropagation(); setColunaEditandoIndex(index); }}
                            ...
                            <div className="flex justify-center my-2">
                              <div
                                {...attributes}
                                {...listeners}
                                className="h-1 w-8 bg-gray-300 rounded-full cursor-move"
