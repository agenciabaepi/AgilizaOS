...
function sanitizeFilename(name: string): string {
  ...
  return base || `fornecedor-${Date.now()}.png`;
}

/** Upload de logo de fornecedor / marca da peça. */
export async function POST(req: NextRequest) {
  try {
    ...
    const filePath = `pecas/fornecedores/${Date.now()}_${safeName}`;
    ...
    if (uploadError) {
      console.error('Upload pecas fornecedor:', uploadError);
      const hint =
