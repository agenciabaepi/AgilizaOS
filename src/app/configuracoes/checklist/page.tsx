...
export default function ChecklistConfigPage() {
  ...
  const executarMigracao = async () => {
    if (!confirm('Isso irá criar a tabela checklist_itens e inserir itens padrão. Continuar?')) {
      return;
    }

    try {
      addToast('info', 'Executando migração...');
      
      const response = await fetch('/api/migrar-checklist', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        addToast('success', `Migração concluída! ${data.itensInseridos} itens inseridos para ${data.empresas} empresas.`);
        fetchItens(true); // Recarregar lista
      } else {
        addToast('error', data.error || 'Erro ao executar migração');
      }
    } catch (error) {
      console.error('Erro ao executar migração:', error);
      addToast('error', 'Erro ao executar migração');
    }
  };
        ...
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-gray-400 mt-3">
              Clique em "Executar Migração" para criar a tabela e inserir itens padrão
            </p>
          </div>
        ) : (
