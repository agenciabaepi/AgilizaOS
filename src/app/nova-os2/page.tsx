...
function NovaOS2Content() {
  ...
  useEffect(() => {
    async function fetchUsuarios() {
      ...
      if (!error && data) {
        ...
        if (usuarioData) {
          const usuarioLogado = data.find(u => u.auth_user_id === usuarioData.empresa_id);
          if (usuarioLogado && usuarioLogado.nivel === 'tecnico') {
            setTecnicoResponsavel(usuarioLogado.id);
          }
        }
      }
  ...
  async function finalizarOS() {
    ...
    try {
      ...
      const tecnicoSelecionado = tecnicos.find(t => t.id === tecnicoResponsavel);
                ...
                <div className="space-y-4">
                  ...
                  <ReactSelect
                    ...
                    value={tecnicoResponsavel ? { 
                      value: tecnicoResponsavel, 
                      label: tecnicos.find(t => t.id === tecnicoResponsavel)?.nome || '' 
                    } : null}
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-black text-white rounded-full flex items-center justify-center text-sm font-medium">
                        {tecnicos.find(t => t.id === tecnicoResponsavel)?.nome.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-700">
                          {tecnicos.find(t => t.id === tecnicoResponsavel)?.nome}
                        </p>
