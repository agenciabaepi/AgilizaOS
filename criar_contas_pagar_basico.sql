-- =====================================================
-- SCRIPT BÁSICO: SISTEMA DE CONTAS A PAGAR
-- =====================================================
-- Este script cria as tabelas necessárias para o sistema de contas a pagar
-- Versão simplificada sem políticas complexas
...
-- =====================================================
-- 5. CRIAR POLÍTICAS DE SEGURANÇA SIMPLES
-- =====================================================
...
CREATE POLICY "contas_select_policy" 
    ON contas_pagar FOR SELECT 
    USING (empresa_id IN (
        SELECT empresa_id FROM usuarios WHERE auth_user_id = auth.uid()
    ));

CREATE POLICY "contas_insert_policy" 
    ON contas_pagar FOR INSERT 
    WITH CHECK (empresa_id IN (
        SELECT empresa_id FROM usuarios WHERE auth_user_id = auth.uid()
    ));

CREATE POLICY "contas_update_policy" 
    ON contas_pagar FOR UPDATE 
    USING (empresa_id IN (
        SELECT empresa_id FROM usuarios WHERE auth_user_id = auth.uid()
    ));

CREATE POLICY "contas_delete_policy" 
