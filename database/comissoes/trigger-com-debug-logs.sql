-- =====================================================
-- TRIGGER COM LOGS DE DEBUG DETALHADOS
-- =====================================================
-- Esta versão adiciona logs para identificar exatamente onde o trigger está falhando

-- 1. RECRIAR A FUNÇÃO COM LOGS
-- =====================================================
...
BEGIN
    ...
    -- LOG 1: Trigger foi chamado
    RAISE WARNING '[TRIGGER] 🔔 Função executada - OS: %, Status: % (normalizado: %), Status Técnico: % (normalizado: %), Data Entrega: %, Técnico: %', 
        NEW.id, NEW.status, status_normalizado, NEW.status_tecnico, status_tecnico_normalizado, NEW.data_entrega, NEW.tecnico_id;
    ...
    IF NOT (
        ...
        RAISE WARNING '[TRIGGER] ❌ NÃO ATENDE CRITÉRIOS - Status: % (normalizado: %), Status Técnico: % (normalizado: %), Data Entrega: %, Técnico: %', 
            NEW.status, status_normalizado, NEW.status_tecnico, status_tecnico_normalizado, NEW.data_entrega, NEW.tecnico_id;
        RETURN NEW;
    END IF;
    
    RAISE WARNING '[TRIGGER] ✅ ATENDE CRITÉRIOS BÁSICOS';
    ...
    IF comissao_existente IS NOT NULL THEN
        RAISE WARNING '[TRIGGER] ⚠️ COMISSÃO JÁ EXISTE - ID: %', comissao_existente;
        RETURN NEW;
    END IF;
    
    RAISE WARNING '[TRIGGER] 🔍 Buscando dados do técnico: %', NEW.tecnico_id;
    ...
    IF NOT FOUND THEN
        RAISE WARNING '[TRIGGER] ❌ TÉCNICO NÃO ENCONTRADO - ID: %', NEW.tecnico_id;
        RETURN NEW;
    END IF;
    
    RAISE WARNING '[TRIGGER] ✅ Técnico encontrado: % (nome), comissao_ativa: %', tecnico_record.id, tecnico_record.comissao_ativa;
    ...
    IF tecnico_record.comissao_ativa = false THEN
        RAISE WARNING '[TRIGGER] ❌ TÉCNICO COM COMISSÃO DESATIVADA - ID: %, Nome: %, comissao_ativa: %', 
            tecnico_record.id, tecnico_record.id, tecnico_record.comissao_ativa;
        RETURN NEW;
    END IF;
    
    RAISE WARNING '[TRIGGER] ✅ Técnico tem comissão ativa, continuando...';
    ...
    IF tecnico_record.tipo_comissao IS NOT NULL THEN
        ...
        END IF;
        RAISE WARNING '[TRIGGER] 📊 Usando configuração individual do técnico - Tipo: %, Valor: %', tipo_comissao_tecnico, valor_comissao_tecnico;
    ELSIF config_comissao_record.tipo_comissao IS NOT NULL THEN
        ...
        END IF;
        RAISE WARNING '[TRIGGER] 📊 Usando configuração padrão da empresa - Tipo: %, Valor: %', tipo_comissao_tecnico, valor_comissao_tecnico;
    ELSE
        tipo_comissao_tecnico := 'porcentagem';
        valor_comissao_tecnico := 10;
        RAISE WARNING '[TRIGGER] 📊 Usando fallback padrão - Tipo: %, Valor: %', tipo_comissao_tecnico, valor_comissao_tecnico;
    END IF;
    ...
    RAISE WARNING '[TRIGGER] 💰 Valor da comissão calculado: % (Valor faturado: %, Tipo: %, Valor base: %)', 
        valor_comissao_calculado, NEW.valor_faturado, tipo_comissao_tecnico, valor_comissao_tecnico;
    ...
    BEGIN
        INSERT INTO comissoes_historico (
            ...
            NULL,
            true
        ) RETURNING id INTO comissao_existente;
        
        RAISE WARNING '[TRIGGER] ✅✅✅ COMISSÃO REGISTRADA COM SUCESSO! ID da comissão: %, ID da OS: %', comissao_existente, NEW.id;
    ...
    EXCEPTION WHEN OTHERS THEN
        ...
        BEGIN
            ...
            RAISE WARNING '[TRIGGER] ✅✅✅ COMISSÃO REGISTRADA COM SUCESSO (sem campo ativa)! ID: %, OS: %', comissao_existente, NEW.id;
            
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING '[TRIGGER] ❌❌❌ ERRO AO REGISTRAR COMISSÃO para OS %: %', NEW.id, SQLERRM;
        END;
    END;
...
SELECT 
    ...
    CASE 
        WHEN prosrc LIKE '%RAISE WARNING%' THEN '✅ Função com logs ativada'
        ELSE '⚠️ Função sem logs'
    END as status_logs
FROM pg_proc
WHERE proname = 'registrar_comissao_automatica';

-- 5. TESTE: ATUALIZAR UMA O.S. PARA VER OS LOGS
-- =====================================================
-- Execute esta query após atualizar uma O.S. para ver os logs do trigger
-- Os logs aparecerão no console do Supabase ou na aba "Logs"
SELECT 'Execute: UPDATE ordens_servico SET updated_at = NOW() WHERE numero_os = 1312;' as instrucao_teste;
